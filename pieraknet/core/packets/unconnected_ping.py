...
class UnconnectedPing(BasePacket):
    ...
    def __init__(self, timestamp: int, client_guid: int, magic: bytes | None = None, packet_id: int = 0x01):
    ...
    @classmethod
    def deserialize(cls, data: bytes | BytesIO, read_packet_id: bool = True) -> "UnconnectedPing":
        ...
        if read_packet_id:
            packet_id, timestamp, magic, client_guid = cast(tuple[int, int, bytes, int], packet_data.data_types)
            if packet_id not in cls.PACKET_IDS:
                raise ValueError("Invalid packet ID")
        else:
            timestamp, magic, client_guid = cast(tuple[int, bytes, int], packet_data.data_types)
